...
import {siteSystemsRouter} from "./routes/routes";
...
class App {
    ...
    private setupRoutes(): void {
        this.server.use(siteSystemsRouter);
    }
    ...
    public async start(port: number): Promise<void> {
        try {
            ...
            this.httpServer = this.server.listen(port, () => {
                console.log(
                    `🚀 ${SERVICE_NAMES.siteSystems} service is running on port ${port}`
                );
            });
...
app.start(BASE_PORTS.siteSystems);